...
def test_for_real(model):
   ...
   meas = scio.loadmat(r'./meas_all.mat')['meas']
   # meas = scio.loadmat(r'./bear_ori.mat')['bear']
   # meas = np.expand_dims(meas,axis=0)
   # plt.imshow((meas-meas.min())/(meas.max()-meas.min()))
   # plt.show()
    #meas = np.stack((meas,meas,meas,meas),axis=0)
   # print(meas.shape,meas.min(),meas.max())
   # assert 0
   ...
   for i in range(1):#meas.shape[0]
        for j in range(20):
            if not os.path.exists(path):                   ########################
                os.makedirs(path)                          ##########################
            cv2.imwrite(r'./result/{}_{}.png'.format('result', j),out_save1.detach().cpu().numpy()[i, j, :, :] * 255)
