...
def get_pc_sampler(sde, shape, predictor, corrector, inverse_scaler, snr,
  ...
  def pc_sampler(model,sautocorr,beta_start,beta_step,beta_stop,N_iter,init_guess,percent,net):
    ...
    with torch.no_grad():
      ...
      x_input = np.zeros((1,2,512,512),dtype=np.float32)
      #x_input = np.zeros((1,2,256,256),dtype=np.float32)
      g1=init_guess                                                                               #init_guess/g1：随机生成一个与原图大小相同的全息图（振幅加相位）
      cor=[]
      ...
      for ibeta in range(len(BETAS)):
        ...
        for i in range(N_iter):
          ...
          #################################################################################
          x_input[0,0,:,:] = np.real(G_uv)
          x_input[0,1,:,:] = np.imag(G_uv)
          ...
          x_mean = x_mean.cpu().numpy() # (1,2,512,512)    
          x_mean = np.array(x_mean,dtype=np.float32)

          G_uv = x_mean[0,0,:,:] + 1j*x_mean[0,1,:,:] 
          #################################################################################
          g1_tag =np.real(np.fft.ifft2(sautocorr*G_uv/(np.abs(G_uv)+(1e-12))))    #sautoxorr: 根号mag    G_uv:Fi[k]   g1_tag:si
          g1=g1_tag*(g1_tag>=0)*S+(g1 - beta*g1_tag)*(g1_tag<0)*S+ (g1 - beta*g1_tag)*(1-S) 
          if i < 15:
            g1 = g1 * (g1_tag >= 0) * S
            Max = np.max(g1)
            g1 = ffdnet_vdenoiser(g1 / (Max+(1e-12)), 5 / 255, net) * Max #tunable parameter of noise level for different input; uncomment for FFDNet
            #g1 = tvdenoise(g1[:,:],10,2)     #TV
            #g1=g1.cpu().numpy()
          g1 = tvdenoise(g1[:,:],10,2)     #TV
          g1=g1.cpu().numpy()
      ...
      for i in range(N_iter):
        '''
        x_input[0,0,:,:] = np.real(g1)
        x_input[0,1,:,:] = np.imag(g1)
